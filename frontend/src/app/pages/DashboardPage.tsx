  ...
  const loadRiskData = useCallback(
    async (lat?: number, lng?: number) => {
      ...
      try {
        ...
        const reportsResult = await fetchReports('approved');

          const incidentReports = (reportsResult.data ?? []).filter((report) => {
            return report.status === 'approved' && report.category === 'BANJIR';
          });

          const nearbyReports = incidentReports.filter((report) => {
            if (
  ...
  return (
    <div className="p-4 lg:p-6 space-y-4 lg:space-y-6 text-[var(--text-main)]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm transition-colors lg:p-4 dark:border-[rgba(255,255,255,0.06)] dark:bg-[#1d2027]">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-slate-500 text-[10px] uppercase tracking-widest font-medium lg:text-xs dark:text-[#8c909f]">
                LAPORAN SEKITAR
              </p>

              <p className="text-slate-950 text-xl font-bold mt-1 lg:text-3xl dark:text-[#e1e2ec]">
                {loading ? '...' : `${dashboardReports.length} Unit`}
              </p>
            ...
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-blue-100 bg-blue-50 dark:border-[rgba(173,198,255,0.16)] dark:bg-[rgba(173,198,255,0.08)]">
              <FileText size={16} className="text-blue-600 dark:text-[#adc6ff]" />
            </div>
          </div>
        </div>
      </div>
