...
export interface CommunityReport {
  ...
  denial_count?: number;
  total_votes?: number;
  current_user_vote?: 'confirm' | 'deny' | null;
  created_at?: string;
  updated_at?: string;
...
export interface UserNotification {
  ...
  type: 'approved_report' | 'rejected_report' | 'nearby_report' | string;
  title: string;
  message: string;
  is_read: boolean;
  created_at?: string;
...
export interface NotificationPreferences {
  user_id: string;
  approved_report: boolean;
  rejected_report: boolean;
...
export async function fetchReports(
  ...
  const url = queryString ? `${API_BASE}/reports?${queryString}` : `${API_BASE}/reports`;
...
export async function fetchMyReports(
  ...
  return parseJsonResponse(response, 'Failed to fetch my reports');
}
...
export async function fetchReportDetail(
  ...
  const response = await fetch(
    `${API_BASE}/reports/${reportId}${queryString ? `?${queryString}` : ''}`,
    {
      method: 'GET',
      headers: { Accept: 'application/json' },
    },
...
export async function voteReport(
  ...
  return parseJsonResponse(response, 'Failed to submit report vote');
}
...
export async function confirmReport(
  ...
  return voteReport(reportId, userId, 'confirm');
}

export async function denyReport(
  reportId: string,
  userId: string,
): Promise<{ success: boolean; data: CommunityReport }> {
  return voteReport(reportId, userId, 'deny');
}
...
export async function fetchUserNotifications(
  ...
  return parseJsonResponse(response, 'Failed to fetch notifications');
}
...
export async function fetchNotificationPreferences(
  ...
  const response = await fetch(
    `${API_BASE}/notification-preferences?user_id=${encodeURIComponent(userId)}`,
    {
      method: 'GET',
      headers: { Accept: 'application/json' },
    },
  ...
  return parseJsonResponse(response, 'Failed to fetch notification preferences');
}

export async function updateNotificationPreferences(
  userId: string,
  patch: Partial<Omit<NotificationPreferencesPayload, 'user_id'>>,
): Promise<{ success: boolean; data: NotificationPreferences }> {
  const response = await fetch(`${API_BASE}/notification-preferences/${userId}`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      Accept: 'application/json',
